/**
 * Factory that converts a gRPC error into the appropriate {@link VoiceError}
 * subclass.
 *
 ...
 * | Everything else                        | VoiceError (base)        |
 */
...
/**
 * Convert a caught gRPC error into the appropriate `VoiceError` subclass.
 *
 * If the error is not a recognised gRPC error, it is wrapped in the base
 * `VoiceError` with a generic `internalError` code.
 */
export function fromGrpcError(error: unknown): VoiceError {
  ...
  const options: VoiceErrorOptions = {
    code: errorCode,
    retryable,
    grpcCode,
    cause,
